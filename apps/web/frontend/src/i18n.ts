import type { Lang } from "./types";

export const i18n = {
  en: {
    statusLoading: "Checking local runtime...",
    apiLinks: "API links",
    workflowTabs: "AgentForge workflows",
    workspaceStatus: "Workspace status",
    inspectableDetails: "Inspectable run details",
    runtime: "Runtime",
    health: "Health",
    skills: "Skills",
    skillCount: "Skills",
    tasksets: "Tasksets",
    tasksetCount: "Tasksets",
    traces: "Traces",
    mode: "Mode",
    runMode: "Run Mode",
    agentMode: "Execution Mode",
    harnessWorkflow: "Workflow Mode",
    toolCallingAgent: "Autonomous Agent",
    localModeHint: "Local deterministic mode",
    providerModeHint: "Model provider mode",
    tabDashboard: "Dashboard",
    tabChat: "Chat",
    tabGenerate: "Generate Skill",
    tabRun: "Run Skill",
    tabEvolve: "Evolve Skill",
    tabRuns: "Runs",
    tabSkills: "Skills",
    tabTasks: "Tasks",
    tabHqs: "HQS",
    tabTraceViewer: "Trace Viewer",
    tabTools: "Tools",
    tabMemory: "Memory",
    tabSettings: "Settings",
    message: "Message",
    run: "Run",
    debug: "Debug",
    requirement: "Requirement",
    generateSkill: "Generate Skill",
    skill: "Skill",
    taskInput: "Task input",
    runSkill: "Run Skill",
    taskset: "Taskset",
    maxIterations: "Max iterations",
    minImprovement: "Min improvement",
    evolveSkill: "Evolve Skill",
    emptyResult: "Run a workflow to see results.",
    hqs: "HQS",
    response: "Response",
    system: "System",
    runSummary: "Run",
    intent: "Intent",
    plan: "Plan",
    trace: "Trace",
    warnings: "Warnings",
    artifacts: "Artifacts",
    none: "None",
    timeline: "Timeline",
    timelineEmpty: "No run yet.",
    debugJson: "Debug JSON",
    running: "Running...",
    progressTitle: "Workflow running",
    elapsed: "Elapsed",
    seconds: "s",
    longTaskHint: "Provider calls can take minutes with thinking models. Keep this tab open while AgentForge writes traces and artifacts.",
    stageSubmit: "Submit request",
    stageModelThinking: "Model thinking",
    stageValidateWrite: "Validate and write artifacts",
    stageSkillExecute: "Run selected Skill",
    stageContractTrace: "Normalize output and write trace",
    stageHqsTrace: "Score HQS and save trace",
    stageRunBaseline: "Run current Skill",
    stageRewriteCandidate: "Rewrite and test candidate",
    stageGatePersist: "Apply quality gate and persist result",
    runIdle: "No active run",
    stop: "Stop",
    traceReady: "Trace",
    noResponse: "No response.",
    generated: "Generated Skill",
    generatedBody: "Skill generated and validated.",
    runOutput: "Skill Output",
    evolveResult: "Evolution Result",
    missingSkills: "No Skills found.",
    missingTasksets: "No tasksets found.",
    providerMode: "provider",
    localMode: "local",
    drilldown: "Drilldown",
    drilldownEmpty: "Run a workflow, then inspect trace, HQS, memory, or Skill diffs.",
    memory: "Memory",
    skillDiff: "Skill Diff",
    loading: "Loading...",
    noTrace: "No trace available.",
    noDiff: "No Skill diff available for this version.",
    retrieval: "Retrieval",
    dimensions: "Dimensions",
    messageEmpty: "Message is empty.",
    requirementEmpty: "Requirement is empty.",
    taskInputEmpty: "Task input is empty.",
    localApiRunning: "Local API running",
    unavailable: "unavailable",
    offline: "offline",
    unknown: "unknown",
    status: "Status",
    runId: "Run",
    modeLabel: "Mode",
    pathLabel: "Path",
    versionLabel: "Version",
    finalSkill: "Final Skill",
    stopReason: "Stop reason",
    iterations: "Iterations",
    decision: "decision",
    candidate: "candidate",
    runDir: "Run",
    runs: "Runs",
    runsSubtitle: "Local run records with task results, steps, artifacts, and quality signals.",
    refresh: "Refresh",
    noRuns: "No runs recorded yet.",
    taskType: "Task Type",
    created: "Created",
    updated: "Updated",
    output: "Output",
    steps: "Steps",
    errors: "Errors",
    latestEpisodes: "Latest Episodes",
    semanticMemory: "Semantic Memory",
    executionState: "Execution State",
    diff: "Diff",
    type: "type",
    schema: "schema",
    completed: "completed",
    failed: "failed",
    skipped: "skipped",
    avg: "avg",
    confidence: "confidence",
    query: "query",
    episodes: "episodes",
    semantic: "semantic",
    score: "score",
    artifact: "artifact",
    artifactCount: "artifact",
    errorCount: "error",
    warning: "Warning",
    step: "step",
    transition: "transition",
    episode: "episode",
    skillSource: "source",
    toolCalls: "Tool Calls",
    workflowCheckpoints: "Workflow Checkpoints",
    currentStep: "Current step",
    earlierCheckpointsHidden: "earlier checkpoints hidden",
    modelDecision: "Model Decision",
    toolArguments: "Tool Arguments",
    validation: "Validation",
    validationErrors: "Validation Errors",
    observation: "Observation",
    parseRepair: "Parse Repair",
    repaired: "repaired",
    repairStrategy: "repair",
    finalAnswerSource: "Final answer",
    repairCount: "repairs",
    hqsGate: "HQS Gate",
    qualityRetry: "Retry",
    codeAnalysis: "Code Analysis",
    findings: "Findings",
    sources: "Sources",
    lines: "Lines",
    high: "High",
    medium: "Medium",
    low: "Low",
    info: "Info",
    limitations: "Limitations",
    recommendation: "Recommendation",
    noFindings: "No findings.",
    source: "Source",
    tools: "Tools",
    toolsSubtitle: "Available tool capabilities with input/output schemas and execution constraints.",
    noTools: "No tools registered.",
    permission: "Permission",
    timeout: "Timeout",
    idempotent: "Idempotent",
    sideEffects: "Side effects",
    inputSchema: "Input Schema",
    outputSchema: "Output Schema",
    errorSpecs: "Error Specs",
    yes: "Yes",
    no: "No",
    memorySubtitle: "Local memory index for episodes, semantic summaries, and retrievable context.",
    search: "Search",
    noEpisodes: "No episode memory.",
    noSemanticMemory: "No semantic memory.",
    tasks: "Tasks",
    tasksSubtitle: "Dispatchable task catalog with request schemas and manual execution.",
    noTaskTypes: "No task types registered.",
    stable: "stable",
    experimental: "experimental",
    optionsSchema: "Options Schema",
    requestJson: "Request JSON",
    runTask: "Run Task",
    taskResult: "Task Result",
    recentRuns: "Recent Runs",
    invalidJson: "Request JSON is invalid.",
    hqsSubtitle: "Quality scores and dimensions for response and system reliability.",
    currentSystemHqs: "Current System HQS",
    lastResponseHqs: "Last Response HQS",
    lastSystemHqs: "Last System HQS",
    selectedRunHqs: "Selected Run HQS",
    noHqsReports: "No HQS reports.",
    noDimensions: "No dimensions.",
    traceViewer: "Trace Viewer",
    traceViewerSubtitle: "Local trace browser for steps, errors, and diagnostic clues.",
    noTraces: "No traces found.",
    traceId: "Trace ID",
    rawTrace: "Raw Trace",
    diagnoseTrace: "Diagnose Trace",
    diagnosis: "Diagnosis",
    dashboard: "Dashboard",
    dashboardTitle: "AgentForge Workbench",
    dashboardSubtitle: "A compact view of Agent runtime, observability, memory, traces, and quality signals.",
    newChat: "New Chat",
    currentRun: "Current Run",
    openRuns: "Open Runs",
    open: "Open",
    noCheckpoint: "No workflow checkpoint.",
    platformSurface: "Workbench Surface",
    phase7CompleteSurface: "Phase 7",
    settings: "Settings",
    settingsSubtitle: "Execution mode, provider configuration, local directories, and API status.",
    projectRoot: "Project Root",
    providerConfig: "Provider Config",
    selectedProvider: "Selected Provider",
    directories: "Directories",
    available: "available",
    missing: "missing",
    writable: "writable",
    readonly: "readonly",
    apiKey: "API Key",
    skillsSubtitle: "Versioned Skill library with Markdown, metadata, and diffs.",
    metadata: "Metadata",
    versionMetadata: "Version Metadata",
    skillMarkdown: "SKILL.md"
  },
  zh: {
    statusLoading: "正在检查本地运行状态...",
    apiLinks: "API 链接",
    workflowTabs: "AgentForge 工作流",
    workspaceStatus: "工作台状态",
    inspectableDetails: "可钻取运行详情",
    runtime: "运行时",
    health: "健康",
    skills: "技能",
    skillCount: "技能",
    tasksets: "任务集",
    tasksetCount: "任务集",
    traces: "追踪",
    mode: "模式",
    runMode: "运行模式",
    agentMode: "运行方式",
    harnessWorkflow: "工作流模式",
    toolCallingAgent: "自主Agent",
    localModeHint: "本地确定性模式",
    providerModeHint: "大模型调用模式",
    tabDashboard: "Dashboard",
    tabChat: "对话",
    tabGenerate: "生成 Skill",
    tabRun: "运行 Skill",
    tabEvolve: "演化 Skill",
    tabRuns: "Runs",
    tabSkills: "Skills",
    tabTasks: "Tasks",
    tabHqs: "HQS",
    tabTraceViewer: "Trace Viewer",
    tabTools: "Tools",
    tabMemory: "Memory",
    tabSettings: "Settings",
    message: "消息",
    run: "运行",
    debug: "调试",
    requirement: "需求",
    generateSkill: "生成 Skill",
    skill: "Skill",
    taskInput: "任务输入",
    runSkill: "运行 Skill",
    taskset: "任务集",
    maxIterations: "最大迭代",
    minImprovement: "最小提升",
    evolveSkill: "演化 Skill",
    emptyResult: "运行一个流程后查看结果。",
    hqs: "HQS",
    response: "响应",
    system: "系统",
    runSummary: "运行摘要",
    intent: "意图",
    plan: "计划",
    trace: "追踪",
    warnings: "警告",
    artifacts: "产物",
    none: "无",
    timeline: "时间线",
    timelineEmpty: "暂无运行。",
    debugJson: "调试 JSON",
    running: "运行中...",
    progressTitle: "流程运行中",
    elapsed: "已耗时",
    seconds: "秒",
    longTaskHint: "Thinking 模型调用可能持续数分钟。请保持当前页面打开，AgentForge 会继续写入 trace 和产物。",
    stageSubmit: "提交请求",
    stageModelThinking: "模型思考",
    stageValidateWrite: "校验并写入产物",
    stageSkillExecute: "执行选中的 Skill",
    stageContractTrace: "规范输出并写入 trace",
    stageHqsTrace: "计算 HQS 并保存 trace",
    stageRunBaseline: "运行当前 Skill",
    stageRewriteCandidate: "重写并测试候选版本",
    stageGatePersist: "执行质量门禁并保存结果",
    runIdle: "暂无运行",
    stop: "停止",
    traceReady: "追踪",
    noResponse: "没有响应。",
    generated: "已生成 Skill",
    generatedBody: "Skill 已生成并通过校验。",
    runOutput: "Skill 输出",
    evolveResult: "演化结果",
    missingSkills: "没有找到 Skill。",
    missingTasksets: "没有找到任务集。",
    providerMode: "大模型",
    localMode: "本地",
    drilldown: "钻取视图",
    drilldownEmpty: "运行一个流程后，可查看追踪、HQS、记忆或 Skill diff。",
    memory: "记忆",
    skillDiff: "Skill Diff",
    loading: "加载中...",
    noTrace: "暂无追踪。",
    noDiff: "当前版本没有 Skill diff。",
    retrieval: "检索结果",
    dimensions: "维度",
    messageEmpty: "消息不能为空。",
    requirementEmpty: "需求不能为空。",
    taskInputEmpty: "任务输入不能为空。",
    localApiRunning: "本地 API 已启动",
    unavailable: "不可用",
    offline: "离线",
    unknown: "未知",
    status: "状态",
    runId: "运行",
    modeLabel: "模式",
    pathLabel: "路径",
    versionLabel: "版本",
    finalSkill: "最终 Skill",
    stopReason: "停止原因",
    iterations: "迭代",
    decision: "决策",
    candidate: "候选",
    runDir: "运行目录",
    runs: "运行记录",
    runsSubtitle: "本地运行记录，支持查看任务结果、步骤、产物和质量信号。",
    refresh: "刷新",
    noRuns: "暂无运行记录。",
    taskType: "任务类型",
    created: "创建时间",
    updated: "更新时间",
    output: "输出",
    steps: "步骤",
    errors: "错误",
    latestEpisodes: "最近事件",
    semanticMemory: "语义记忆",
    executionState: "执行状态",
    diff: "Diff",
    type: "类型",
    schema: "Schema",
    completed: "完成",
    failed: "失败",
    skipped: "跳过",
    avg: "均分",
    confidence: "置信度",
    query: "查询",
    episodes: "事件",
    semantic: "语义",
    score: "分数",
    artifact: "产物",
    artifactCount: "产物",
    errorCount: "错误",
    warning: "警告",
    step: "步骤",
    transition: "状态变更",
    episode: "事件",
    skillSource: "来源",
    toolCalls: "工具调用",
    workflowCheckpoints: "Workflow Checkpoints",
    currentStep: "当前步骤",
    earlierCheckpointsHidden: "个更早的 checkpoint 已隐藏",
    modelDecision: "模型决策",
    toolArguments: "工具参数",
    validation: "校验",
    validationErrors: "校验错误",
    observation: "观察",
    parseRepair: "解析修复",
    repaired: "已修复",
    repairStrategy: "修复策略",
    finalAnswerSource: "最终响应",
    repairCount: "修复次数",
    hqsGate: "HQS 门禁",
    qualityRetry: "质量重试",
    codeAnalysis: "代码分析",
    findings: "发现项",
    sources: "来源",
    lines: "行数",
    high: "高",
    medium: "中",
    low: "低",
    info: "信息",
    limitations: "限制",
    recommendation: "建议",
    noFindings: "没有发现项。",
    source: "来源",
    tools: "工具",
    toolsSubtitle: "可用工具能力，包含输入输出结构和执行约束。",
    noTools: "没有已注册工具。",
    permission: "权限",
    timeout: "超时",
    idempotent: "幂等",
    sideEffects: "副作用",
    inputSchema: "输入 Schema",
    outputSchema: "输出 Schema",
    errorSpecs: "错误规格",
    yes: "是",
    no: "否",
    memorySubtitle: "本地记忆索引，用于查看历史事件、语义摘要和可检索上下文。",
    search: "搜索",
    noEpisodes: "暂无 episode memory。",
    noSemanticMemory: "暂无 semantic memory。",
    tasks: "任务",
    tasksSubtitle: "可调度任务目录，支持查看请求结构并手动触发任务。",
    noTaskTypes: "暂无已注册任务类型。",
    stable: "稳定",
    experimental: "实验性",
    optionsSchema: "选项 Schema",
    requestJson: "请求 JSON",
    runTask: "运行任务",
    taskResult: "任务结果",
    recentRuns: "最近运行",
    invalidJson: "请求 JSON 格式无效。",
    hqsSubtitle: "质量评分与维度明细，用于判断响应和系统运行是否可靠。",
    currentSystemHqs: "当前系统 HQS",
    lastResponseHqs: "最近响应 HQS",
    lastSystemHqs: "最近系统 HQS",
    selectedRunHqs: "选中运行的 HQS",
    noHqsReports: "暂无 HQS 报告。",
    noDimensions: "暂无维度。",
    traceViewer: "Trace Viewer",
    traceViewerSubtitle: "本地 Trace 浏览器，用于追踪步骤、错误和诊断线索。",
    noTraces: "暂无 trace。",
    traceId: "Trace ID",
    rawTrace: "原始 Trace",
    diagnoseTrace: "诊断 Trace",
    diagnosis: "诊断结果",
    dashboard: "Dashboard",
    dashboardTitle: "AgentForge 工作台",
    dashboardSubtitle: "集中查看 Agent 运行、观测、记忆、Trace 和质量信号。",
    newChat: "新对话",
    currentRun: "当前运行",
    openRuns: "打开 Runs",
    open: "打开",
    noCheckpoint: "暂无 workflow checkpoint。",
    platformSurface: "工作台能力面",
    phase7CompleteSurface: "Phase 7",
    settings: "设置",
    settingsSubtitle: "运行方式、Provider 配置、本地目录和 API 状态。",
    projectRoot: "项目根目录",
    providerConfig: "Provider 配置",
    selectedProvider: "选中 Provider",
    directories: "目录",
    available: "可用",
    missing: "缺失",
    writable: "可写",
    readonly: "只读",
    apiKey: "API Key",
    skillsSubtitle: "版本化 Skill 库，支持查看 Markdown、元数据和版本差异。",
    metadata: "元数据",
    versionMetadata: "版本元数据",
    skillMarkdown: "SKILL.md"
  }
} as const;

export type I18nKey = keyof typeof i18n.en;

export function translate(lang: Lang, key: I18nKey): string {
  return i18n[lang][key] || i18n.en[key] || key;
}
