...
- 运行索引和状态使用本地 SQLite 保存到 `data/agentforge.db`。
...
工具定义和执行能力已经正式迁到 `src/agentforge/tools/`：

- `schema.py` 定义 `AgentTool`、`ToolSchema`、`ToolCall` 和 `ToolResult`。
- `registry.py` 负责工具注册、查询和统一执行入口。
- `executor.py` 集中处理输入 / 输出 schema 校验、权限校验、timeout、异常标准化和 `tool_calls` 持久化。
- `src/agentforge/agent/tools.py` 仅保留为兼容导出层，旧导入路径仍可使用。

多步骤任务编排已经通过 `src/agentforge/workflows/` 抽象出统一入口：

- `definition.py` 定义 workflow、step、stop / retry / artifact policy。
- `runner.py` 包装 RunService，统一处理 start、step execution、checkpoint、fail、complete。
- `checkpoint.py` 把 workflow 状态写入 SQLite `workflow_checkpoints`。
- chat、tool-calling chat、Skill generation、Skill run、Skill evolve 已接入 workflow-aware runner。
- `trace_diagnosis`、`code_analysis`、`document_analysis`、`data_analysis` 已迁入 `WorkflowRunner.execute()`，由统一 step handler 编排负责 retry、step、artifact、trace 和 checkpoint 收口。

统一任务路由已经通过 `src/agentforge/tasks/` 建立：

- `schemas.py` 定义 `TaskRequest`、`TaskTypeSpec` 和 `TaskResult`。
- `router.py` 负责列出任务类型并把请求分发给具体 handler。
- `handlers/skill_tasks.py` 复用现有 Skill generate / run / evolve 能力。
- `handlers/trace_diagnosis.py` 会读取 trace 或 run 关联 trace，生成结构化诊断，并写入 run、workflow checkpoint 和 `trace_diagnosis` trace。
- `handlers/code_analysis.py` 提供本地确定性的代码分析，会读取显式文件 / 目录 / 代码块，生成风险、语法、可维护性 finding，并写入 run、workflow checkpoint 和 `code_analysis` trace。
- `handlers/document_analysis.py` 提供本地文本 / Markdown 文档分析，会提取标题、章节、链接、代码块和文档质量 finding，并写入 run、workflow checkpoint 和 `document_analysis` trace。
- `handlers/data_analysis.py` 提供本地 CSV / TSV / JSON / JSONL 数据画像，会生成行列数、缺失值、数值列统计和数据质量 finding，并写入 run、workflow checkpoint 和 `data_analysis` trace。
- 当前可执行任务类型：`skill_generate`、`skill_run`、`skill_evolve`、`trace_diagnosis`、`code_analysis`、`document_analysis`、`data_analysis`。

聊天入口也已经接入任务路由：

- `parse_intent` 会输出 `task_type`、`task_input` 和 `task_options`。
- `harness_workflow` 的 `build_plan -> execute_plan` 会把 `trace_diagnosis`、`code_analysis`、`document_analysis`、`data_analysis` 和可执行 `skill_*` 任务交给 Task Router。
- `/api/chat` 的紧凑响应会返回 `intent.task_type` 和 `task_result`，因此调用方不必手动调用 `/api/tasks` 才能触发 trace 诊断或代码分析。
...
AgentForge 会自动初始化本地 SQLite 文件 `data/agentforge.db`。它不需要 MySQL、PostgreSQL 或独立数据库服务。
...
统一任务入口可用于后续 Workflow Runner 和 Task Router 共用的任务执行。示例：诊断最新 trace。
...
curl http://127.0.0.1:8765/api/tools/execute_plan
```

查看分层 memory：

```bash
curl http://127.0.0.1:8765/api/memory/episodes
curl http://127.0.0.1:8765/api/memory/semantic?q=ui
```
...
curl -X POST http://127.0.0.1:8765/api/chat ^
  -H "Content-Type: application/json" ^
  -d "{\"message\":\"Analyze src/agentforge/agent/intent_parser.py for edge cases.\"}"
```
